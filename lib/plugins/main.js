...
const configSource = <%= typeof options.config === 'string' %> ? require('<%= options.config %>') : <%= serialize(options.config) %>
...
function writeImportStatement(service, staticImport = false) {
  ...
  let pathAppendix = ''
  if (serviceOptions && serviceOptions.memoryOnly) {
...
export default async (ctx, inject) => {
  const res = ctx.res
  const config = 'default' in configSource ? configSource.default(ctx) : configSource
