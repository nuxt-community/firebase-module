...
export default async (ctx, inject) => {
  ...
  const fire = {
    ...
    async ready() {
      ...
      const servicePromises = [
        <%= serverServices.reduce((acc, service) => `${acc}fire.${service.injectionName}Ready(),\n        `, '') %>
      ]

      <% if (clientServices.length) { %>
      if (process.client) {
        Array.prototype.push.apply(servicePromises, [
          <%= clientServices.reduce((acc, service) => `${acc}fire.${service.injectionName}Ready(),\n          `, '') %>
        ])
      }
      <% } %>
  ...
  <% const serviceName = service.injectionName %>
  ...
  const fire = {
    <%= enabledServices.map(service => `${service.injectionName}: ${service.id}`).join(',\n    ') %>
  }
