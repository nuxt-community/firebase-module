...
export default async (ctx, inject) => {
  ...
  const fire = {
    ...
    async ready() {
      ...
      const servicePromises = [
        <%= serverServices.reduce((acc, service) => `${acc}fire.${serviceNames[service]}Ready(),\n        `, '') %>
      ]

      <% if (clientServices.length) { %>
      if (process.client) {
        Array.prototype.push.apply(servicePromises, [
          <%= clientServices.reduce((acc, service) => `${acc}fire.${serviceNames[service]}Ready(),\n          `, '') %>
        ])
      }
      <% } %>
  ...
  fire.<%= serviceName %>Ready = async () => {
    if (!fire.<%= serviceName %>) {
      await fire.appReady()
      fire.<%= serviceName %> = await <%= `${service}Service(session, firebase, ctx, inject)` %>
    }
  ...
  const servicePromises = [
    <%= serverServices.reduce((acc, service) => `${acc}${service}Service(session, firebase, ctx, inject),\n    `, '') %>
  ]

  <% if (clientServices.length) { %>
  if (process.client) {
    Array.prototype.push.apply(servicePromises, [
      <%= clientServices.reduce((acc, service) => `${acc}${service}Service(session, firebase, ctx, inject),\n      `, '') %>
    ])
  }
  <% } %>
