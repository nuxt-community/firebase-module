...
module.exports = async function (moduleOptions) {
  ...
  if (messagingSW || authSW) {
    this.nuxt.hook('build:before', () => {
      ...
      const pluginPath = resolve(compiledDir, 'workbox-extension-plugin.js')
      const pluginCompiled = template(
        readFileSync(resolve(templatesDir, 'workbox-extension-plugin.js'))
      )
      ...
      this.options.pwa = {
        ...
        workbox: {
          ...workbox,
          swTemplate: resolve(templatesDir, 'service-worker.js'),
          offline: true,
          workboxExtensions: [
            pluginPath,
      ...
      if (messagingSW) {
        ...
        const messagingPath = resolve(
          compiledDir,
          'workbox-extension-messaging.js'
        )
        const messagingCompiled = template(
          readFileSync(resolve(templatesDir, 'workbox-extension-messaging.js'))
        )
        writeFileSync(
          messagingPath,
          messagingCompiled({
      ...
      if (authSW) {
        ...
        const authPath = resolve(compiledDir, 'workbox-extension-auth.js')
        const authCompiled = template(
          readFileSync(resolve(templatesDir, 'workbox-extension-auth.js'))
        )
        writeFileSync(
          authPath,
          authCompiled({
