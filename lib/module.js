...
module.exports = function (moduleOptions) {
  ...
  if (auth && !isEmpty(auth.initialize)) {
    ...
    if (auth.initialize.ssr) {
      ...
      const credential =
        typeof auth.initialize.ssr === 'boolean'
          ? null
          : typeof auth.initialize.ssr === 'string'
            ? this.resolver.resolveAlias(auth.initialize.ssr)
            : auth.initialize.ssr

      this.addTemplate({
        src: resolve(__dirname, 'serverMiddleware/firebaseServerAuth.js'),
        fileName,
        options: {
          credential,
          config: options.config
      ...
      this.addServerMiddleware(join(this.options.buildDir, fileName))
      // Add Service-Worker
      this.addTemplate({
