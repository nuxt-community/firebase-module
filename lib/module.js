...
module.exports = async function (moduleOptions) {
  ...
  if (messagingSW || authSW) {
    this.nuxt.hook('build:before', () => {
      ...
      mkdirSync(resolve(__dirname, 'compiled'))

      const pluginPath = resolve(
        __dirname,
        'compiled/workbox-extension-plugin.js'
      )
      const pluginCompiled = template(
        readFileSync(
          resolve(__dirname, 'templates/workbox-extension-plugin.js')
        )
      )
      ...
      if (messagingSW) {
        ...
        const messagingPath = resolve(
          __dirname,
          'compiled/workbox-extension-messaging.js'
        )
        const messagingCompiled = template(
          readFileSync(
            resolve(__dirname, 'templates/workbox-extension-messaging.js')
          )
        )
        writeFileSync(
          messagingPath,
      ...
      if (authSW) {
        ...
        const authPath = resolve(
          __dirname,
          'compiled/workbox-extension-auth.js'
        )
        const authCompiled = template(
          readFileSync(
            resolve(__dirname, 'templates/workbox-extension-auth.js')
          )
        )
        writeFileSync(
          authPath,
