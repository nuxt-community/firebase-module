...
module.exports = function (moduleOptions) {
  ...
  if (auth.ssr) {
    ...
    if (ssrConfig.serverLogin && ssrConfig.credential) {
      this.addPlugin({
      ...
      const sessionLifetime = ssrConfig.sessionLifetime || 0
      this.nuxt.hook('render:routeDone', async (_, __, { res }) => {
        if (res && res.locals && res.locals._firebase) {
          const firebase = res.locals._firebase

          const elapsed = Date.now() - firebase.options._created
