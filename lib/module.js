...
module.exports = function (moduleOptions) {
  ...
  if (this.nuxt.options.dev && !('legacyMode' in this.options.firebase) && !('legacyMode' in moduleOptions)) {
    logger.info('A legacyMode has been introduced in v7 which will by default be false in v8. Set \'legacyMode: true\' in your config to remove this message or migrate to the new API. Read more here: XX')
  }
  ...
  // Use serviceMapping to keep key order
  // TODO: is this reliable?
  for (const service in templateUtils.serviceMapping) {
    ...
    this.addTemplate({
      src: r(`plugins/services/${service}.js`),
      fileName: `firebase/${service === 'app' ? '' : 'service.'}${service}.js`,
      options: {
        ...options,
        ...templateUtils,
        service,
  ...
  this.addPlugin({
    ...
    options: {
      ...
      ...templateUtils,
      enabledServices,
      clientOnlyServices: ['analytics', 'messaging', 'performance', 'remoteConfig']
    }
  })
